...
add_image_feed(request) - Представление для добавления изображений. Если метод запроса — POST,
он проверяет данные формы, создает новый экземпляр ImageFeed, сохраняет изображения с широтой и долготой по умолчанию и
обновляет местоположение, если данные EXIF присутствуют. В случае успеха перенаправляется на страницу панели мониторинга.
Если метод запроса не POST, он отображает форму.
