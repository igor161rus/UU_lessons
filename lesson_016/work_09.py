...
print(f"{Album.select().where(Album.publisher == 'Atlantic')[0].title}")
print(f"{Album.get(Album.title == 'Led Zeppelin IV').publisher}")

for album in Album.select().join(Artist):
    print(album.artist.name, album.title, album.release_date)
    # album.title += '!'
Album.delete().where(Album.id > 30).execute()
# album.save()
