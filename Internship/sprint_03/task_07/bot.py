...
def resize_for_sticker(image):
    """
    Изменяет размер изображения для стикера.
    Args:
    image (PIL.Image): Изображение для изменения размера.
    Returns:
...
def send_welcome(message):
    bot.reply_to(message, comp.get_compliment())
...
def get_options_keyboard(message):
    ...
    if user_states.get(message.chat.id) and user_states[message.chat.id]['level'] == 0:
        ...
        keyboard.add(pixelate_btn, ascii_btn, mirror_btn, heatmap_btn, resize_btn)
        user_states[message.chat.id]['message_id'] = message.message_id
        return keyboard
...
def pixelate_and_send(message):
    ...
    elif user_states.get(message.chat.id) and user_states[message.chat.id]['level'] == 9:
        pixelated = resize_image(image)
